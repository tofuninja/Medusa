...
public class Pan extends JPanel {
	...
	public void setDiag(Diagram d) 
	{
		...
		for (int i = 0; i < d.JavaBlocks.size(); i++) 
		{
			...
			String str = db.getClassName();
			
			for(int j = 0; j < db.getMethodNames().size(); j++)
			{
				str += db.getMethodNames().get(j) + "\n";
			}
			
			str += "\n";
			
			for(int j = 0; j < db.getVariableNames().size(); j++)
			{
				str += db.getVariableNames().get(j) + "\n";
			}
			
			
			Block b = new Block(str, (int) db.getX(), (int) db.getY(),new Color(196, 121, 126), font);
			block_list.add(b);
		}
